...
- 이처럼 화면 UI만으로 알기 어려운 부분을 설명하면, 작업자들은 이 내용을 보면서 자신의 업무를 유추할 수 있다.
...
- 화면을 나누었으면 슬라이드 위 또는 아래에 컷아웃(cutout) 도형을 삽입해서 화면이 서로 연결된다는 의미를 작업자에게 전달한다.
...
### 2. 웹 사이트 맵의 대분류를 반드시 네비게이션 메뉴로 해야하는건 아니다.
